    ...
    fn extract_block(
        ...
        match node.expr() {
            SemExpression::Block(values) => {
                ...
                let finished_block = format!(
                    "Block<{real_ty}> block_{} = {header_format};\n",
                    self.block_count
                );
        ...
        match node.expr() {
            ...
            SemExpression::FieldAccess(left, right) => {
                ...
                let right = match right.expr() {
                    SemExpression::Id(name) => name.to_string(),
                    _ => self.translate_expr(right, scope, ctx)?,
                };
                ...
                match &e.ty() {
                    Type::YourType(x) => {
                        if records.contains_key(x) {
                            ...
                            Err(CompilerError::BackendError(format!(
                                "Cannot destructure expression of type {}",
                                e.ty(),
                            )))
                        }
                    }
                    Type::EnumType(parent, child) => {
                        ...
                        match found {
                            ...
                            EnumField::Id(_) => Err(CompilerError::BackendError(format!(
                                "Cannot destructure expression of type {} because it has no fields",
                                e.ty(),
                            ))),
                        }
                    }
                ...
                let mut rest: Vec<_> = rest
                    ...
                    .map(|f| {
                        ...
                        let co = match co.expr() {
                            SemExpression::EnumLiteral(parent, child) => match child.expr() {
                        ...
                        let bo = match bo.expr().clone() {
                            SemExpression::Block(_) => {
                                ...
                                let (block, ret) =
                                    self.extract_block(&bo, &mut sc, real_ty, ctx).unwrap();
                                let ret = format!("{ret};\n");
            ...
            SemExpression::Lets(ids, expr) => {
                ...
                for lt in ids {
                    ...
                    let a =
                        self.translate_val(name.as_str(), &exprl, &mut sc, real_ty.clone(), ctx)?;
                    s.push_str("\t\t");
                    s.push_str(&a);
                }
            ...
            SemExpression::Decimal(val, t) => match *t.clone() {
